...
# def stochastic_gradient_descent(X, y, gamma=1e-3, n_iter=100_000):
#     """
#     Implémentation de la descente de gradient stochastique pour minimiser l'erreur quadratique.
    
#     Paramètres:
#     -----------
#     X : numpy.ndarray
#         Matrice des caractéristiques (features).
#     y : numpy.ndarray
#         Vecteur des labels correspondants.
#     gamma : float
...
#     --------
#     w : numpy.ndarray
#         Vecteur de poids appris.
#     """
#     w = np.random.randn(X.shape[1])  # Initialisation aléatoire des poids
    
#     for _ in range(n_iter):
#         # Sélection d'un échantillon aléatoire
#         idx = np.random.randint(low=0, high=X.shape[0], size=10)
#         X_i = X[idx, :]
#         y_i = y[idx]
        
#         # Calcul du gradient
#         gradient = X_i.T @ (X_i @ w.T - y_i) / len(idx)
